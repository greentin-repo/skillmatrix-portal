...
(function ( angular ) {
	...
	module.factory("snackbar", ['$rootScope', function ($rootScope) {
		...
		return {
			create : function ( content, duration, snackbarType ) { 

				$rootScope.$broadcast('createSnackbar', { 'content': content, 'duration': duration, 'type' : snackbarType });
			}
		}
	}]);
		...
		return function ( scope, element, attrs ) {
			...
			$rootScope.$on('createSnackbar', function(event, received) {
				...
				if(received.type == "success") {
					snackbarType = "snackbar-success";
				} else if(received.type == "error") {
					snackbarType = "snackbar-error";
				} else {
					snackbarType = "snackbar-default";
				}
				var template = "<div class=\"snackbar snackbar-opened "+snackbarType+"\"><span class=\"snackbar-content\">" + received.content + "</span></div>";

				//template compile
				var snackbar = angular.element($compile(template)(scope));

				//add snackbar
				snackbarContainer.empty();
