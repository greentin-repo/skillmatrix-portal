...
var ctrl = app.controller("employeeCtrl", ['$scope', '$filter', 'appService', '$location', 'snackbar', '$route', '$rootScope', 'CONST_RESOURCES', '$translate', function($scope, $filter, appService, $location, snackbar, $route, $rootScope, CONST_RESOURCES, $translate) {
    ...
    $scope.addSetupDetails = function() {
        ...
        $scope.flagexcelimport = true;
        var name = '';
        if ($rootScope.empDetails.firstName) {
            name += $rootScope.empDetails.firstName;
        }
        if ($rootScope.empDetails.middleName) {
            if ($rootScope.empDetails.firstName) {
                name += ' ';
            }
            name += $rootScope.empDetails.middleName;
        }
        if ($rootScope.empDetails.lastName) {
            if ($rootScope.empDetails.firstName || $rootScope.empDetails.middleName) {
                name += ' ';
            }
            name += $rootScope.empDetails.lastName;
        }
        ...
        formData.append("branchId", $scope.empDet.uploadSelectBranchId);
            ...
            .then(function(response) {
                ...
                if (response.data.result) {
                    ...
                    if (response.data.statusCode == 100) {
                        ...
                        $("#upload-file-input")
                            .val('');
                        if (response.data.errorList != null && response.data.errorList.length > 0) {
                            $scope.erroEmployeeList = response.data.errorList;
                            $('#employee_errorlist')
                                .modal('show');
                        } else {
    ...
    $scope.createEmployee = function() {
        $scope.step = 1;
        $scope.empInputData = {};
        $scope.empInputData.fname = '';
        $scope.empInputData.mname = '';
        $scope.empInputData.lname = '';
        $scope.empInputData.contactno = '';
        $scope.empInputData.empid = ''
        $scope.empInputData.emailId = '';
        $scope.empInputData.designation = '';
        $scope.empInputData.cmpyEmpId = '';
        $scope.form.addemployeeForm.$setPristine();
        $scope.form.addemployeeForm.$setUntouched();
        // $scope.add = '';
    ...
    $scope.setDate = function(type) {
        if (type == 'dob') {
            if ($scope.dob != null && !angular.isUndefined($scope.empInputData.dob)) {
                $('.datepicker_dob')
                    .datepicker('setDate', $scope.empInputData.dob);
                $('.datepicker_dob')
                    .datepicker('update');
            }
            //console.log('Enter DOB')
        } else if (type == 'doj') {
            if ($scope.doj != null && !angular.isUndefined($scope.empInputData.doj)) {
                $('.datepicker_doj')
                    .datepicker('setDate', $scope.empInputData.doj);
                $('.datepicker_doj')
                    .datepicker('update');
            }
            console.log('Enter DOJ')
        } else if (type == 'doa') {
            if ($scope.doa != null && !angular.isUndefined($scope.empInputData.doa)) {
                $('.datepicker_doa')
                    .datepicker('setDate', $scope.empInputData.doa);
                $('.datepicker_doa')
                    .datepicker('update');
            }
            //console.log('Enter DOB')
    ...
    $scope.submitEmployee = function(form) {
        console.log(form.$invalid)
        if (form.$invalid) {
            console.log("when form is invalid")
        ...
        if ($scope.empInputData.emailId == '' && $scope.empInputData.contactno == '' && $scope.empInputData.cmpyEmpId == '') {
            snackbar.create($scope.langTranslate.Pleaseemployee_ID, 3000, "error");
            return;
        }
        var lineFlag = 'N';
        if ($scope.lineNameList != null && $filter('filter')($scope.lineNameList, { isActive: '!' + lineFlag }, true).length > 0) {
            if ($scope.empInputData.line == undefined || $scope.empInputData.line == null || $scope.empInputData.line == '') {
                snackbar.create('Please select line', 3000, "error");
                return;
            }
        }
        // if ($scope.empInputData.doj == undefined || $scope.empInputData.doj == null || $scope.empInputData.doj == '') {
        //     snackbar.create('Please select date of joining', 3000, "error");
        //     return;
        // }
        if ($scope.emplevelList != null && $scope.emplevelList.length > 0) {
            if ($scope.empInputData.emplvlId == undefined || $scope.empInputData.emplvlId == null || $scope.empInputData.emplvlId == '') {
        ...
        if (form.$valid && validDropdown) {
            $scope.flagAddSpinner = true;
            $scope.data = {
                cmpyEmpId: $scope.empInputData.cmpyEmpId,
                firstName: $scope.empInputData.fname,
                middleName: $scope.empInputData.mname,
                lastName: $scope.empInputData.lname,
                contactNo: $scope.empInputData.contactno,
                createdBy: $rootScope.empDetails.empId,
                dept: {
                    deptId: $scope.empInputData.dept.deptId
                },
                organization: {
                ...
                branch: {
                    branchId: $scope.empInputData.createSelectBranchId
                },
                emailId: $scope.empInputData.emailId,
                designation: $scope.empInputData.designation,
                ...
                empLevelDetails: {
                    empLvlId: parseInt($scope.empInputData.emplvlId)
                }
            };

            if ($scope.empInputData.line != null) {
                $scope.data.line = $scope.empInputData.line.id;
            }
            if ($scope.empInputData.doa == null || $scope.empInputData.doa == 'null' || $scope.empInputData.doa == '' || angular.isUndefined($scope.empInputData.doa)) {
                //delete $scope.data.doa;
            } else {
                $scope.data.doa = $scope.empInputData.doa;
            }
            if ($scope.empInputData.dob == null || $scope.empInputData.dob == 'null' || $scope.empInputData.dob == '' || angular.isUndefined($scope.empInputData.dob)) {
                //delete $scope.data.dob;
            } else {
                $scope.data.dob = $scope.empInputData.dob;
            }
            if ($scope.empInputData.doj == null || $scope.empInputData.doj == 'null' || $scope.empInputData.doj == '' || angular.isUndefined($scope.empInputData.doj)) {
                //delete $scope.data.dob;
            } else {
                $scope.data.doj = $scope.empInputData.doj;
            }

            if ($scope.empInputData.emplvlId == 0 || $scope.empInputData.emplvlId == undefined) {
            ...
            console.log("when form is invalid")
            angular.forEach($scope.form.addemployeeForm.$error.required, function(field) {
                field.$setDirty();
            });
        }
    };
    ...
    $scope.editEmployee = function(x) {
        ...
        if (x.line != null) {
            ...
            $scope.empInputData.line = x.line;
        }
        if (x.branch.branchId != null) {
            $scope.empInputData.branchId = x.branch.branchId.toString();
            // $scope.brachName = x.branch.name;
        }

        if (x.empLevelDetails != null) {
            // $scope.empLvlDet = x.empLevelDetails.empLvlId.toString();
            $scope.empInputData.empLvlDet = x.empLevelDetails.empLvlId.toString();
        } else {
            $scope.empInputData.empLvlDet = '';
        }
        ...
        $scope.getDepartmentList($scope.empInputData.branchId);
        $scope.getemployeeLevelDetails($scope.empInputData.branchId);
        //$scope.getBranchList();
        $("#employee_editModal1")
            .modal({
                backdrop: 'static',
    ...
    $scope.editEmpDet = function(form, isValid) {
        ...
        $scope.doj = form.doj.$modelValue;
        // console.log($scope.empLvlDet);
        if ($scope.empInputData.branchId == null || $scope.empInputData.branchId == '' || angular.isUndefined($scope.empInputData.branchId)) {
            snackbar.create("Please Select Branch", 3000, "error");
            return
        }
        if ($scope.empInputData.emailId == '' && $scope.empInputData.contactNo == '' && $scope.empInputData.cmpyEmpId == '') {
        ...
        if ($scope.empInputData.deptId == null || $scope.empInputData.deptId == '' || angular.isUndefined($scope.empInputData.deptId)) {
        ...
        // }
        if ($scope.emplevelList != null && $scope.emplevelList.length > 0) {
            if ($scope.empInputData.empLvlDet == undefined || $scope.empInputData.empLvlDet == null || $scope.empInputData.empLvlDet == '') {
                snackbar.create('Please select employee level', 3000, "error");
                return;
            }
        }
        ...
        if (isValid) {
            ...
            $scope.data = {
                "empId": parseInt($scope.empInputData.empId),
                "firstName": $scope.empInputData.firstName,
                "middleName": $scope.empInputData.middleName,
                "lastName": $scope.empInputData.lastName,
                "contactNo": $scope.empInputData.contactNo,
                "deptId": $scope.empInputData.deptId,
                "branchId": $scope.empInputData.branchId,
                "cmpyEmpId": $scope.empInputData.cmpyEmpId,
                "emailId": $scope.empInputData.emailId,
                "designation": $scope.empInputData.designation,
                "isSetupCompleted": $rootScope.empDetails.productOrgConfigDet.isSetupCompleted,
                "registrationByPass": $rootScope.empDetails.productOrgConfigDet.registrationByPass,
                "line": $scope.empInputData.line,
                "empLevelDetails": {
                    "empLvlId": parseInt($scope.empInputData.empLvlDet)
                }
            };
    ...
    $scope.newSetupUserDet = {};
    $scope.setDeptDesigOfRegUser = function() {
        //console.log($scope.selectedDeptId)
        if (angular.isUndefined($scope.newSetupUserDet.selectedDeptId)) {
            // snackbar.create("Please select department.", 3000, "error");
        ...
        $scope.data = {
            ...
            "deptId": parseInt($scope.newSetupUserDet.selectedDeptId),
            "branchId": $rootScope.empDetails.branch.branchId,
        ...
        appService.httpPost($scope.data, "updateemployeedetails")
            .then(function(response) {
                ...
                //console.log(response);
                if (response.result) {
                    var dept = $scope.getDeptName($scope.newSetupUserDet.selectedDeptId);
            ...
            if ($scope.employeeList != null && $scope.employeeList.length > 0) {
                for (var i = 0; i < $scope.employeeList.length; i++) {
                    if ($scope.filterByStatus($scope.employeeList[i])) {
                        ...
                        var name = '';
                        if ($scope.employeeList[i].firstName) {
                            name += $scope.employeeList[i].firstName;
                        }
                        if ($scope.employeeList[i].middleName) {
                            if ($scope.employeeList[i].firstName) {
                                name += ' ';
                            }
                            name += $scope.employeeList[i].middleName;
                        }
                        if ($scope.employeeList[i].lastName) {
                            if ($scope.employeeList[i].firstName || $scope.employeeList[i].middleName) {
                                name += ' ';
                            }
                            name += $scope.employeeList[i].lastName;
                        }
                        exportDataTmp.push({
                            ...
                            "Mobile No": ($scope.employeeList[i].contactNo != null) ? $scope.employeeList[i].contactNo : '-',
                            "Department": deptName,
                            ...
                            "DOJ": ($scope.employeeList[i].doj != null) ? $scope.employeeList[i].doj : '-',
                            "Status": status
                        });
                    }
                }
            }
            console.log(exportDataTmp);
        ...
        if ($scope.transferFunDet.hasOwnProperty('response') && !$scope.isEmpty($scope.transferFunDet.response)) {
            for (let index = 0; index < $scope.transferFuncationalityMenu.length; index++) {
                if ($scope.transferFuncationalityMenu[index].menuName == 'Suggestions') {
                    ...
                    $scope.transferFuncationalityMenu[index].count = $scope.getCountNArray('count', 'concernPendingCount');
                    ...
                    $scope.transferFuncationalityMenu[index].count = $scope.getCountNArray('count', 'surveyPendingCount');
    ...
    $scope.submitTransferEmp = function(form) {
        ...
        var req = {
            ...
            // if ($scope.lineNameList != null && $filter('filter')($scope.lineNameList, { isActive: '!' + N }, true).length > 0) {
        if (!$scope.isEmpty($scope.transferEmpData.line)) {
            req.toLine = $scope.transferEmpData.line.id;
        }
        console.log(req);
