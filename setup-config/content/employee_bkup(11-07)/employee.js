...
var ctrl = app.controller("employeeCtrl", ['$scope', '$filter', 'appService', '$location', 'snackbar', '$route', '$rootScope', 'CONST_RESOURCES', '$translate', function ($scope, $filter, appService, $location, snackbar, $route, $rootScope, CONST_RESOURCES, $translate) {
    ...
    $scope.submitEmployee = function (form) {
        ...
        //  && $scope.empInputData.cmpyEmpId == ''
        if ($scope.empInputData.emailId == '' && $scope.empInputData.contactno == '') {
        ...
        if (form.$valid && validDropdown) {
            ...
            if ($scope.empInputData.cmpyEmpId) {
                $scope.data.cmpyEmpId = $scope.empInputData.cmpyEmpId;
            }
            if ($scope.empInputData.emailId) {
                $scope.data.emailId = $scope.empInputData.emailId;
            }
            if ($scope.empInputData.mname) {
                $scope.data.middleName = $scope.empInputData.mname;
            }
            if ($scope.empInputData.lname) {
                $scope.data.lastName = $scope.empInputData.lname;
            }
            if ($scope.empInputData.contactno) {
                $scope.data.contactNo = $scope.empInputData.contactno;
            }
            if ($scope.empInputData.designation) {
                $scope.data.designation = $scope.empInputData.designation;
            }
            ...
            if ($scope.empInputData.empAddr != "") {
                $scope.data.address = $scope.empInputData.empAddr;
            }
    ...
    $scope.editEmployee = function (x) {
        ...
        if (x.address == undefined || x.address == null) {
            $scope.empInputData.empAddress = "";
        } else {
            $scope.empInputData.empAddress = x.address;
        }
        $scope.updatedept = {}
        if (x.dept != null) {
    ...
    $scope.editEmpDet = function (form, isValid) {
        ...
        if (isValid) {
            ...
            if ($scope.empInputData.middleName) {
                $scope.data.middleName = $scope.empInputData.middleName;
            }
            if ($scope.empInputData.lastName) {
                $scope.data.lastName = $scope.empInputData.lastName;
            }
            if ($scope.empInputData.contactNo) {
                $scope.data.contactNo = $scope.empInputData.contactNo;
            }
            ...
            if ($scope.empInputData.line) {
                $scope.data.line = $scope.empInputData.line;
            }
            ...
            // }
            if ($scope.empInputData.empAddress) {
                $scope.data.address = $scope.empInputData.empAddress
            }
            //console.log($scope.data);
            // return;
